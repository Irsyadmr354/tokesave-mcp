...
const BINARY_EXTS = new Set(['.png', '.jpg', '.jpeg', '.gif', '.ico', '.svg', '.woff', '.woff2', '.eot', '.ttf', '.otf', '.pdf', '.zip', '.gz', '.tar', '.exe', '.dll', '.so', '.dylib', '.bin', '.dat', '.a', '.lib', '.obj', '.pdb', '.o', '.pyc', '.class', '.jar']);
...
function run(cmd, cwd) {
  try {
    ...
    return execSync(cmd, { encoding: 'utf8', maxBuffer: 10 * 1024 * 1024, cwd: dir }).trim();
  } catch {
    return '';
  }
}
...
function auditRepo(repoPath) {
  ...
  const files = run('git ls-files', repoRoot).split('\n').filter(Boolean);
  if (files.length === 0) return 'Not a git repo or no tracked files.';

  const commitCount = run('git log --oneline', repoRoot).split('\n').filter(Boolean).length;
  const totalSize = files.reduce((sum, f) => sum + (fs.existsSync(path.join(repoRoot, f)) ? fs.statSync(path.join(repoRoot, f)).size : 0), 0);
  ...
  if (binaryFiles.length > 0) {
    ...
    for (const f of shown) {
      const sz = fs.existsSync(path.join(repoRoot, f)) ? fs.statSync(path.join(repoRoot, f)).size : 0;
  ...
  if (generatedFiles.length > 0) {
    output += `\n⚠ Generated/lock files (${generatedFiles.length}): ~${Math.ceil(generatedFiles.reduce((s, f) => s + (fs.existsSync(path.join(repoRoot, f)) ? fs.statSync(path.join(repoRoot, f)).size : 0), 0) / 8)} tokens\n`;
  ...
  const totalWaste = Math.ceil(wasteTokens + generatedFiles.reduce((s, f) => s + (fs.existsSync(path.join(repoRoot, f)) ? fs.statSync(path.join(repoRoot, f)).size : 0) / 8, 0));
